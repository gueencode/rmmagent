...
func New(logger *logrus.Logger, version string) *WindowsAgent {
	host, _ := ps.Host()
	info := host.Info()
	pd := filepath.Join(os.Getenv("ProgramFiles"), "gueenAgent")
	exe := filepath.Join(pd, "gueenrmm.exe")
	...
	k, err := registry.OpenKey(registry.LOCAL_MACHINE, `SOFTWARE\gueenrmm`, registry.ALL_ACCESS)
	if err == nil {
...
// RecovergueenAgent should only be called from the rpc service
func (a *WindowsAgent) RecovergueenAgent() {
	svc := "gueenagent"
	a.Logger.Debugln("Attempting gueenagent recovery on", a.Hostname)
	...
	a.Logger.Debugln("gueenagent recovery completed on", a.Hostname)
}
...
func (a *WindowsAgent) RecoverRPC() {
	...
	_, _ = CMD("net", []string{"stop", "gueenrpc"}, 90, false)
	time.Sleep(2 * time.Second)
	_, _ = CMD("net", []string{"start", "gueenrpc"}, 90, false)
}
...
func (a *WindowsAgent) RecoverCMD(command string) {
	...
	// and not gueenrmm.exe so that we don't kill ourself
	cmd := exec.Command("cmd.exe")
	cmd.SysProcAttr = &windows.SysProcAttr{
		CreationFlags: windows.DETACHED_PROCESS | windows.CREATE_NEW_PROCESS_GROUP,
...
func (a *WindowsAgent) UninstallCleanup() {
	registry.DeleteKey(registry.LOCAL_MACHINE, `SOFTWARE\gueenrmm`)
...
func ShowStatus(version string) {
	...
	svcs := []string{"gueenagent", "gueenrpc", "mesh agent"}
	...
	if window != 0 {
		...
		var handle w32.HWND
		msg := fmt.Sprintf("Agent: %s\n\nRPC Service: %s\n\nMesh Agent: %s", statusMap["gueenagent"], statusMap["gueenrpc"], statusMap["mesh agent"])
		w32.MessageBox(handle, msg, fmt.Sprintf("Gueen RMM v%s", version), w32.MB_OK|w32.MB_ICONINFORMATION)
	} else {
		fmt.Println("Gueen RMM Version", version)
		fmt.Println("Agent:", statusMap["gueenagent"])
		fmt.Println("RPC Service:", statusMap["gueenrpc"])
...
func (a *WindowsAgent) installerMsg(msg, alert string, silent bool) {
	...
	if !silent && window != 0 {
		...
		w32.MessageBox(handle, msg, "Gueen RMM", flags)
	} else {
		fmt.Println(msg)
	}
...
func (a *WindowsAgent) AgentUpdate(url, inno, version string) {
	...
	if err != nil {
		...
		CMD("net", []string{"start", "gueenrpc"}, 10, false)
		return
	}
	if r.IsError() {
	...
	dir, err := ioutil.TempDir("", "gueenrmm")
	if err != nil {
	...
	innoLogFile := filepath.Join(dir, "gueenrmm.txt")
...
func (a *WindowsAgent) setupNatsOptions() []nats.Option {
	...
	opts = append(opts, nats.Name("gueenrmm"))
...
func (a *WindowsAgent) CleanupAgentUpdates() {
	...
	folders, err := filepath.Glob("gueenrmm*")
	if err == nil {
...
func (a *WindowsAgent) RunPythonCode(code string, timeout int, args []string) (string, error) {
	content := []byte(code)
	dir, err := ioutil.TempDir("", "gueenpy")
	if err != nil {
		a.Logger.Debugln(err)
		return "", err
	}
...
func (a *WindowsAgent) GetPython(force bool) {
	...
	url := fmt.Sprintf("https://github.com/gueencode/rmmagent/releases/download/v%s/%s", a.Version, archZip)
	url2 := fmt.Sprintf("https://files.gueenrmm.io/%s", archZip)
...
func (a *WindowsAgent) deleteOldgueenServices() {
...
func (a *WindowsAgent) RunMigrations() {
	a.deleteOldgueenServices()
	CMD("schtasks.exe", []string{"/delete", "/TN", "gueenrmm_fixmesh", "/f"}, 10, false)
}
...
func (a *WindowsAgent) CreategrmmTempDir() {
	// create the temp dir for running scripts
	dir := filepath.Join(os.TempDir(), "grmm")
	if !FileExists(dir) {
